...
### annotation
scATAC.high.combine <- RenameIdents(
  object = scATAC.data.high.rmDoublets,
  '0' = 'Endothelial','1' = 'B','2' = 'Hepatocyte','3' = 'Macrophage/Dendritic','4' = 'Macrophage/Dendritic',
  '5' = 'T','6' = 'T','7' = 'Hepatocyte','8' = 'NK','9' = 'Macrophage/Dendritic',
  '10' = 'HSC','11' = 'Hepatocyte','12' = 'Hepatocyte','13' = 'Hepatocyte',
  '14' = 'Neutrophil','15' = 'Hepatocyte','16' = 'Hepatocyte',
  '17' = 'Cholangiocyte','18' = 'Macrophage/Dendritic'
)

### Co-embedding
DefaultAssay(scATAC.high.combine) ="ACTIVITY"
transfer.anchors2 <- FindTransferAnchors(reference = scRNA, 
                                         query = scATAC.high.combine, 
                                         features = VariableFeatures(object = scRNA),
                                         reference.assay = "RNA",
                                         query.assay = "ACTIVITY", 
                                         reduction = "cca") 
...
imputation <- TransferData(anchorset = transfer.anchors2, refdata = refdata, weight.reduction = scATAC.high.combine[["lsi"]], dims = 2:30)
scATAC.high.combine[["RNA"]] <- imputation
DefaultAssay(scATAC.high.combine) <- "RNA"     

scRNA.object <- AddMetaData(scRNA.object, metadata = rep("scRNA", nrow(scRNA.object@meta.data)), col.name = "Datatype")
scATAC.object <- AddMetaData(scATAC.high.combine, metadata = rep("scATAC", nrow(scATAC.high.combine@meta.data)), col.name = "Datatype")
